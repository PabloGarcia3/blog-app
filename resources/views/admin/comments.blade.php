  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title">You are about to delete comment for {{ $comment->post->title }}.</h4>
      </div>
      <div class="modal-body">
        <p>Are you sure you want to delete comment for {{ $comment->post->title }}?</p>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-default" data-dismiss="modal">No, Cancel</button>
        <form id="adminDeleteComment-{{ $comment->id }}" method="POST" action="{{ route('adminDeleteComment', $comment->id) }}">@csrf
